"""DINOv2-Giant @ 336 — third backbone for ensemble (CNN + EVA-02 ViT + DINOv2-G ViT).

DINOv2 has different pretraining (self-supervised on LVD-142M) than EVA-02
(masked image modeling on Merged-38M), so even though both are ViTs, their
feature spaces diverge enough to add ensemble diversity.

Output: dinov2g_best_ema.pt, dinov2g_live_best.pt, dinov2g_swa.pt
"""
...
# DINOv2-Giant — patch 14, native 224, but timm interpolates pos_embed for 336.
# 336 is a good fine-grained sweet spot for medical imagery.
MODEL_NAME = "vit_giant_patch14_dinov2.lvd142m"
IMG_SIZE = 336
BATCH_SIZE = 24                    # 1.1B params @ 336 — adjust if OOM (try 16, then 12)

EMA_OUTPUT = "dinov2g_best_ema.pt"
LIVE_OUTPUT = "dinov2g_live_best.pt"
SWA_OUTPUT = "dinov2g_swa.pt"

SEED = 300
EPOCHS = 25
LR = 5.0e-5                        # lower than EVA-02 — DINOv2-G is bigger and more sensitive
WEIGHT_DECAY = 0.05
GRAD_CLIP = 1.0
LABEL_SMOOTHING = 0.1
WARMUP_FRAC = 0.1
MIXUP_OFF_FRAC = 0.2
EMA_DECAY = 0.999                  # learned from ConvNeXt run — 0.9999 too slow for 25 epochs w/ fresh head
SWA_START_FRAC = 0.8
...
print("  (downloading ~5 GB of weights on first run)")
...
def save_ckpt(path, sd, acc, f1, epoch):
    torch.save({
        "model_state_dict": sd,
...
for epoch in range(EPOCHS):
    ...
    if live_f1 > best_live_f1:
        ...
        save_ckpt(LIVE_OUTPUT, model.state_dict(), live_acc, live_f1, epoch)
        print(f"  saved live → {LIVE_OUTPUT}")
        improved = True
    if ema_f1 > best_ema_f1:
        best_ema_f1 = ema_f1; best_ema_acc = ema_acc
        save_ckpt(EMA_OUTPUT, ema.module.state_dict(), ema_acc, ema_f1, epoch)
        print(f"  saved ema  → {EMA_OUTPUT}")
        improved = True
...
if swa_started:
    ...
    save_ckpt(SWA_OUTPUT, swa_model.module.state_dict(), swa_acc, swa_f1, EPOCHS - 1)
    print(f"  saved → {SWA_OUTPUT}")

print(f"\nDone.")
print(f"  Best LIVE F1={best_live_f1:.4f} acc={best_live_acc*100:.2f}%  →  {LIVE_OUTPUT}")
print(f"  Best EMA  F1={best_ema_f1:.4f} acc={best_ema_acc*100:.2f}%  →  {EMA_OUTPUT}")
